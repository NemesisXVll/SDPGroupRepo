...
const ForgetOTP: React.FC = () => {
  ...
  const verifyOTP = () => {
    ...
    } else if (generatedOTP && otp === generatedOTP) { // Compare the entered OTP with the generated OTP
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 h-screen w-full">
      ...
      <div className="bg-gray-100 flex flex-col justify-center">
        ...
        >
          ...
          <div className="mt-4">
            <input type="email" placeholder="Enter Email" value={email} onChange={handleEmailChange} className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
          </div>
          ...
          <div className="mt-7">
            <button type="button" onClick={sendOTP} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
              Send
            </button>
          </div>
