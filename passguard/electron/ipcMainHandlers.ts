...
export const registerIPCMainHandlers = () => {
	...
	ipcMain.on("getPasswordByCredentialIdRequest", async (event, arg) => {
		const data = JSON.parse(JSON.stringify(await userQueryService.getDataByCredentialId(arg)))
		const result = JSON.parse(data.data)
		const password = result.password
    event.sender.send("getPasswordByCredentialIdResponse", JSON.stringify(password));
	});
	...
	ipcMain.on("getReusedPasswordsCountByUserIdRequest", async (event, arg) => {
		event.sender.send(
			...
			JSON.stringify(await userQueryService.getReusedPasswordsCountByUserId(arg))
		);
	});
};
